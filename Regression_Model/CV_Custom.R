...
x <- Predictions$Chlor_2
y <- Predictions$prediction_2

RMSE <- round(sqrt( sum( (y - x)^2 , na.rm = TRUE ) / length(y)), 3)

Correla <-  round(cor(x, y, use="complete"), 3)

one_to_one <- qplot(x,y) +
  geom_abline(intercept = 0, colour = "red", size = 1) +
  geom_smooth(method = "lm", se = FALSE) +
  annotate("text", x = 1, y = 0, size = 5,label =paste0("R2: ", Correla )) +
  annotate("text", x = 1, y = .5, size = 5,label =paste0("RMSE: ", RMSE )) +
